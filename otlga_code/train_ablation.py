...
class SimpleSentenceContrastive(nn.Module):
    ...
    def forward(self, v_features, t_features):
        ...
        """
        t_global = t_features[:, 0, :]  # [B, d] 使用第一个token，与正常训练一致
...
def train_ablation(config_name, config, resume_from=None, epochs=10):
    ...
    print("\n" + "=" * 70)
    print(f"消融实验: {config['name']} ({config_name})")
    print("=" * 70)
    print(f"配置: LGA={config['use_lga']}, OT={config['use_ot']}, Gated Fusion={config['use_gated_fusion']}")
    ...
    optimizer = optim.AdamW(model.parameters(), lr=lr, weight_decay=0.01)  # 与正常训练保持一致
    ...
    if resume_from and os.path.exists(resume_from):
        ...
        if isinstance(checkpoint, dict) and 'model_state_dict' in checkpoint:
            ...
            print(f"  已加载模型状态 (Epoch: {start_epoch}, Best Val Loss: {best_val_loss:.4f})")
        else:
    ...
    for epoch in range(start_epoch, start_epoch + epochs):
        ...
        for batch in pbar:
            ...
            loss_sent = sent_contrast(v_final, t_fused)  # 与正常训练保持一致，不使用attention_mask
        ...
        with torch.no_grad():
            for batch in tqdm(val_loader, desc=f"Epoch {epoch+1}/{epochs} [Val]"):
                ...
                # 验证阶段不包含diversity loss，与正常训练保持一致
