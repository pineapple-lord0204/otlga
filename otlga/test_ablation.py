...
def evaluate_ablation(model, dataloader, device, config_name):
    ...
    print(f"\nExtracting Features [{config_name}]...")
    ...
    print(f"Feature Extraction Complete [{config_name}]")
    ...
    print(f"\n{'='*60}")
    print(f"[{config_name}] Image-to-Text Retrieval")
    print(f"{'='*60}")
    ...
    print(f"\n{'='*60}")
    print(f"[{config_name}] Text-to-Image Retrieval")
    print(f"{'='*60}")
    ...
    print(f"\n{'='*60}")
    print(f"[{config_name}] Average Retrieval Performance")
    print(f"{'='*60}")
...
def main():
    ...
    parser.add_argument('--config', type=str, default='all',
                       ...
                       help='Ablation experiment configuration to test')
    ...
    print("=" * 70)
    print("Ablation Experiment Testing")
    print("=" * 70)
    ...
    for config_name, config in configs_to_test.items():
        ...
        if not os.path.exists(checkpoint_path):
            print(f"\n⚠️  Configuration {config_name}  model does not exist: {checkpoint_path}")
            print("   Please run training first: python3 train_ablation.py --config {config_name}")
            continue

        print(f"\n{'='*70}")
        print(f"Testing Configuration: {config['name']} ({config_name})")
        print(f"{'='*70}")
    ...
    print("\n" + "=" * 70)
    print("Ablation Experiment Testing Summary")
    print("=" * 70)
    print(f"\n{'Configuration':<25} {'Avg R@1':<12} {'Avg R@5':<12} {'Avg R@10':<12}")
    print("-" * 70)
